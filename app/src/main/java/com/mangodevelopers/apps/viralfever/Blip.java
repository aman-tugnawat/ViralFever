...
public class Blip
{
    ...
    public Blip(int width, int height)
    {
        ...
        velocity_x = 0;//2 * Math.sin(rotation);
        velocity_y = 0;//2 * Math.cos(rotation);

        randomColor();
    }
    public Blip(int x, int y, int playerTurn)
    {
        this.width = width;
        this.height = height;

        this.x = x; //random.nextInt(width - radius*2) + radius;
        this.y = y;//random.nextInt(height - radius*2) + radius;
